    ...
    if (checkPermission($userInfo["id"], DELETE)) {
        if ($cmd == "DELETE") {
            ...
            if (!empty($id)) {
                ...
                // get account_id of the Investor
                $account_id = getInvestorAccountId($id);
                ...
                if ($numrows <= 0) {
                    ...
                    // Delete investor
                    $sql = "delete from investor where id=\"" . mysql_real_escape_string($id) . "\"";
                    $result = mysql_query($sql, $conn) or die(mysql_error());

                    setMessage("Record has been deleted.", true);
                    system_log(DELETE, "Investor ID: $id deleted.", $userInfo["id"]);
                    unset($id);
...
<table border="0" width="90%" cellpadding="0" cellspacing="0" align="center">
    <tr>
        ...
        <td class="bg_title"><img src="images/customers.png" width="31" height="20" border="0" align="absmiddle"
                                  style="margin-bottom: 7px;"/> Investor
        </td>
    ...
    <tr>
        ...
        <td>
            <?= showError() ?>
            <p class="box_title">Manage Investor</p>
            <? if (checkPermission($userInfo["id"], VIEW)) { ?>
                <form action="index.php" method="get" autocomplete="off">
                    ...
                    <p style="float: left; width: auto;">
                        ...
                        <input type="text" name="investor_name" id="investor_name" value="<?= $investor_name ?>"/>
                    </p>
                    <p style="float: left; width: auto;">
                ...
                <table border="0" width="100%" cellpadding="5" cellspacing="0" align="center" id="reclist"
                    ...
                    <?
                        if (!empty($investor_name) || !empty($nic_number)) {
                            $special = " WHERE ";
                        }

                        if (!empty($investor_name)) {
                            $special .= " full_name LIKE \"%" . mysql_real_escape_string($investor_name) . "%\"";
                        }
                        ...
                        $sql = "select * from investor " . $special . " order by full_name";
                        $result = mysql_query($sql, $conn) or die(mysql_error());
                        $numrows = mysql_num_rows($result);
                        ...
                        if ($numrows > 0) {
                            ...
                            while ($rs = mysql_fetch_array($result)) {
                                ...
                                if ($canModify) {
                                    echo "<a href='index.php?ss=$ss&mod=investor.add&id=$id&cmd=EDIT'>Modify</a> | ";
                                }
                                ...
                                if ($canView) {
                                    echo "<a href='index.php?ss=$ss&mod=investor.view&investor_id=$id'>View</a>";
                                }
