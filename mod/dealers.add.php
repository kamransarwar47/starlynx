<?
    ...
    if(checkPermission($userInfo["id"], CREATE)) {
        if($cmd == "ADD") {
            if(!empty($full_name) && !empty($father_name) && !empty($id_num) && !empty($date_of_birth) && !empty($street) && !empty($city) && !empty($country) && !empty($commission) && !empty($commission_type)) {
                $sql = "select id from dealers where id_num=\"".mysql_real_escape_string($id_num)."\"";
                ...
                if($numrows <= 0) {
                    ...
                    $sql = "insert into
                                dealers (
                                    ...
                                    id_num, date_of_birth, commission, commission_type, created, created_by
                                ) values (
                                    ...
                                    \"".mysql_real_escape_string($commission_type)."\",
                                    NOW(),
                                    \"".$userInfo["id"]."\"
                                )";
                    $result = mysql_query($sql, $conn) or die(mysql_error());
                    ...
                    unset($cmd, $account_id, $full_name, $father_name, $street, $city, $zip, $state, $country, $phone_1, $phone_2, $mobile_1, $mobile_2, $fax_1, $fax_2, $email_1, $email_2, $id_num, $date_of_birth, $commission, $commission_type);
                } else {
    ...
    if(checkPermission($userInfo["id"], MODIFY)) {
        if($cmd == "UPDATE") {
            if(!empty($id) && !empty($full_name) && !empty($father_name) && !empty($id_num) && !empty($date_of_birth) && !empty($street) && !empty($city) && !empty($country) && !empty($commission) && !empty($commission_type)) {
                if($new_account == "Y") {
                        // Create new account
                        $title = $full_name;
                        $sql = "insert into
                ...
                $sql = "update
                        ...
                        set
                            ...
                            commission=\"".mysql_real_escape_string($commission)."\",
                            commission_type=\"".mysql_real_escape_string($commission_type)."\",
                            updated=NOW(),
                ...
                unset($account_id, $full_name, $father_name, $street, $city, $zip, $state, $country, $phone_1, $phone_2, $mobile_1, $mobile_2, $fax_1, $fax_2, $email_1, $email_2, $id_num, $date_of_birth, $commission, $commission_type);
                $cmd = "EDIT";
            } else {
                setMessage("One or more required fields are empty.");
        ...
        if($cmd == "EDIT") {
            ...
            if(!empty($id)) {
                $sql = "select * from dealers where id=\"".mysql_real_escape_string($id)."\"";
                $result = mysql_query($sql, $conn) or die(mysql_error());
                $numrows = mysql_num_rows($result);
...
<table border="0" width="90%" cellpadding="0" cellspacing="0" align="center">
    <tr>
        ...
        <td class="bg_title"><img src="images/customers.png" width="31" height="20" border="0" align="absmiddle" style="margin-bottom: 7px;" /> Dealers</td>
    ...
    <tr>
        ...
        <td>
            <?=showError()?>
                ...
                <p class="box_title"><?=($cmd=="ADD"?"Create New":"Modify")?> Dealer</p>
                    ...
                    <tr>
                        <td colspan="3">
                            <p>
                                ...
                                <input type="text" size="20" name="commission" id="commission" maxlength="10" value="<?=$commission?>" />
                                <input type="radio" name="commission_type" value="P" <?=($commission_type=="P")?"checked='checked'":""?> /> %
                                <input type="radio" name="commission_type" value="F" <?=($commission_type=="F")?"checked='checked'":""?> /> Fixed
                            </p>
                        </td>
                    </tr>
                </table>
