<?
    ...
    if(checkPermission($userInfo["id"], CREATE)) {
        if($cmd == "ADD") {
            if(!empty($full_name) && !empty($father_name) && !empty($id_num) && !empty($date_of_birth) && !empty($street) && !empty($city) && !empty($country)) {
                $sql = "select id from investor where id_num=\"".mysql_real_escape_string($id_num)."\"";
    ...
    if(checkPermission($userInfo["id"], MODIFY)) {
        ...
        if($cmd == "EDIT") {
            ...
            if(!empty($id)) {
                $sql = "select * from investor where id=\"".mysql_real_escape_string($id)."\"";
                $result = mysql_query($sql, $conn) or die(mysql_error());
                $numrows = mysql_num_rows($result);
...
<table border="0" width="90%" cellpadding="0" cellspacing="0" align="center">
    <tr>
        ...
        <td class="bg_title"><img src="images/customers.png" width="31" height="20" border="0" align="absmiddle" style="margin-bottom: 7px;" /> Investor</td>
    ...
    <tr>
        ...
        <td>
            ...
            <form action="index.php" method="post">
                ...
                <p class="box_title"><?=($cmd=="ADD"?"Create New":"Modify")?> Investor</p>
