    ...
    if (checkPermission($userInfo["id"], DELETE)) {
        if ($cmd == "DELETE") {
            ...
            if (!empty($id)) {
                ...
                // Check Customer Plot Booking
                $sql = "select id from plots where customer_id=\"" . mysql_real_escape_string($id) . "\"";
                $result = mysql_query($sql, $conn) or die(mysql_error());
                $numrows = mysql_num_rows($result);
                if ($numrows <= 0) {

                    // get account_id of the customer
                    $sql = "SELECT account_id FROM customers WHERE id = '{$id}'";
                    $result = mysql_query($sql, $conn) or die(mysql_error());
                    ...
                    if ($numrows <= 0) {
                        ...
                        // Delete customer
                        $sql = "delete from customers where id=\"" . mysql_real_escape_string($id) . "\"";
                        $result = mysql_query($sql, $conn) or die(mysql_error());

                        setMessage("Record has been deleted.", true);
                        system_log(DELETE, "Customer ID: $id deleted.", $userInfo["id"]);
                        unset($id);
                    ...
                    setMessage("Record can not be deleted. (Remove Customer From Plot Booking)");
                    system_log(DELETE, "Operation failed. [Reason: Record can not be deleted. (Remove Customer From Plot Booking)]", $userInfo["id"]);
                }
...
<table border="0" width="90%" cellpadding="0" cellspacing="0" align="center">
    <tr>
        ...
        <td class="bg_title"><img src="images/customers.png" width="31" height="20" border="0" align="absmiddle"
                                  style="margin-bottom: 7px;"/> Customers
        </td>
    ...
    <tr>
        ...
        <td>
            <?= showError() ?>
            <p class="box_title">Manage Customers</p>
            <? if (checkPermission($userInfo["id"], VIEW)) { ?>
                <form action="index.php" method="get" autocomplete="off">
                    ...
                    <p style="float: left; width: auto;">
                        ...
                        <input type="text" name="nic_number" id="nic_number" value="<?= $nic_number ?>" placeholder="xxxxx-xxxxxxx-x"/>
                    </p>
                ...
                <table border="0" width="100%" cellpadding="5" cellspacing="0" align="center" id="reclist"
                    ...
                    <?
                        if (!empty($customer_name) || !empty($nic_number)) {
                            $special = " WHERE ";
                        }

                        if (!empty($customer_name)) {
                            $special .= " full_name LIKE \"%" . mysql_real_escape_string($customer_name) . "%\"";
                        }
                        ...
                        $sql = "select * from customers " . $special . " order by full_name";
                        $result = mysql_query($sql, $conn) or die(mysql_error());
                        $numrows = mysql_num_rows($result);
                        ...
                        if ($numrows > 0) {
                            ...
                            while ($rs = mysql_fetch_array($result)) {
                                ...
                                if ($canModify) {
                                    echo "<a href='index.php?ss=$ss&mod=customers.add&id=$id&cmd=EDIT'>Modify</a> | ";
                                }
                                ...
                                if ($canView) {
                                    echo "<a href='index.php?ss=$ss&mod=customers.view&customer_id=$id'>View</a>";
                                }
