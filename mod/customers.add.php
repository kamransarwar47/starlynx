<?
    ...
    if(checkPermission($userInfo["id"], CREATE)) {
        if($cmd == "ADD") {
            if(!empty($full_name) && !empty($father_name) && !empty($id_num) && !empty($date_of_birth) && !empty($street) && !empty($city) && !empty($country)) {
                $sql = "select id from customers where id_num=\"".mysql_real_escape_string($id_num)."\"";
                ...
                if($numrows <= 0) {
                    if($new_account == "Y") {
                        // Create new account
                        $title = "$full_name s/o $father_name";
                        $sql = "insert into
                                    accounts (
    ...
    if(checkPermission($userInfo["id"], MODIFY)) {
        ...
        if($cmd == "EDIT") {
            ...
            if(!empty($id)) {
                $sql = "select * from customers where id=\"".mysql_real_escape_string($id)."\"";
                $result = mysql_query($sql, $conn) or die(mysql_error());
                $numrows = mysql_num_rows($result);
    ...
    <tr>
        ...
        <td>
            <?=showError()?>
                ...
                <p class="box_title"><?=($cmd=="ADD"?"Create New":"Modify")?> Customer</p>
