<?
    ...
    if(checkPermission($userInfo["id"], CREATE)) {
        if($cmd == "ADD") {
            if(!empty($customer_id) && !empty($relationship) && !empty($full_name) && !empty($father_name) && !empty($id_num) && !empty($date_of_birth) && !empty($street) && !empty($city) && !empty($country)) {
                $sql = "select id from customers_nominees where id_num=\"".mysql_real_escape_string($id_num)."\" and customer_id=\"".mysql_real_escape_string($customer_id)."\"";
                ...
                if($numrows <= 0) {
                    $sql = "insert into
                                customers_nominees (
                                    ...
                                    full_name, father_name, street, city, zip, state, country,
                    ...
                    unset($cmd, $customer_id, $relationship, $full_name, $father_name, $street, $city, $zip, $state, $country, $phone_1, $phone_2, $mobile_1, $mobile_2, $fax_1, $fax_2, $email_1, $email_2, $id_num, $date_of_birth);
                } else {
    ...
    if($cmd == "UPDATE") {
            if(!empty($id) && !empty($customer_id) && !empty($relationship) && !empty($full_name) && !empty($father_name) && !empty($id_num) && !empty($date_of_birth) && !empty($street) && !empty($city) && !empty($country)) {
                $sql = "update
                            customers_nominees
                        set
                ...
                unset($customer_id, $relationship, $full_name, $father_name, $street, $city, $zip, $state, $country, $phone_1, $phone_2, $mobile_1, $mobile_2, $fax_1, $fax_2, $email_1, $email_2, $id_num, $date_of_birth);
                $cmd = "EDIT";
            } else {
                setMessage("One or more required fields are empty.");
        ...
        if($cmd == "EDIT") {
            ...
            if(!empty($id)) {
                $sql = "select * from customers_nominees where id=\"".mysql_real_escape_string($id)."\"";
                $result = mysql_query($sql, $conn) or die(mysql_error());
                $numrows = mysql_num_rows($result);
    ...
    <tr>
        ...
        <td>
            <?=showError()?>
                ...
                <p class="box_title"><?=($cmd=="ADD"?"Create New":"Modify")?> Customer Nominee</p>
                    ...
                    <tr>
                        <td colspan="2" valign="top">
                            <p>
                                ...
                                <select name="customer_id" id="customer_id">
                                    <?
                                        $sql = "select id, full_name, id_num from customers order by full_name";
                                        $result = mysql_query($sql, $conn) or die(mysql_error());
                                        ...
                                        while($rs = mysql_fetch_array($result)) {
                                            ...
                                            echo "<option value=\"".$rs["id"]."\" $selected>".$rs["full_name"]." (".$rs["id_num"].")</option>";
                                        }
                                    ?>
                                </select>
                            </p>
                        ...
                        <td valign="top">
                            <p>
                                ...
                                <input type="text" size="30" name="relationship" id="relationship" maxlength="30" value="<?=$relationship?>" />
                            </p>
                        </td>
                    </tr>
                    <tr>
