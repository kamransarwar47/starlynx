...
if (checkPermission($userInfo["id"], DELETE)) {
    if ($cmd == "DELETE") {
        ...
        if (!empty($id)) {
            ...
            // Check Dealer Plot Booking
            $sql = "select id from plots where dealer_id=\"" . mysql_real_escape_string($id) . "\"";
            $result = mysql_query($sql, $conn) or die(mysql_error());
            $numrows = mysql_num_rows($result);
            if ($numrows <= 0) {

                // get account_id of the dealer
                $sql = "SELECT account_id FROM dealers WHERE id = '{$id}'";
                $result = mysql_query($sql, $conn) or die(mysql_error());
                ...
                if ($numrows <= 0) {
                    ...
                    // Delete dealer
                    $sql = "delete from dealers where id=\"" . mysql_real_escape_string($id) . "\"";
                    $result = mysql_query($sql, $conn) or die(mysql_error());

                    setMessage("Record has been deleted.", true);
                    system_log(DELETE, "Dealer ID: $id deleted.", $userInfo["id"]);
                    unset($id);
                ...
                setMessage("Record can not be deleted. (Remove Dealer From Plot Booking)");
                system_log(DELETE, "Operation failed. [Reason: Record can not be deleted. (Remove Dealer From Plot Booking)]", $userInfo["id"]);
            }
...
<table border="0" width="90%" cellpadding="0" cellspacing="0" align="center">
    <tr>
        ...
        <td class="bg_title"><img src="images/customers.png" width="31" height="20" border="0" align="absmiddle"
                                  style="margin-bottom: 7px;"/> Dealers
        </td>
    ...
    <tr>
        ...
        <td>
            <?= showError() ?>
            <p class="box_title">Manage Dealers</p>
            <? if (checkPermission($userInfo["id"], VIEW)) { ?>
                <form action="index.php" method="get" autocomplete="off">
                    ...
                    <p style="float: left; width: auto;">
                        ...
                        <input type="text" name="dealer_name" id="dealer_name" value="<?= $dealer_name ?>"/>
                    </p>
                    <p style="float: left; width: auto;">
                ...
                <table border="0" width="100%" cellpadding="5" cellspacing="0" align="center" id="reclist"
                    ...
                    <?
                    if (!empty($dealer_name) || !empty($nic_number)) {
                        $special = " WHERE ";
                    }

                    if (!empty($dealer_name)) {
                        $special .= " full_name LIKE \"%" . mysql_real_escape_string($dealer_name) . "%\"";
                    }
                    ...
                    $sql = "select * from dealers " . $special . " order by full_name";
                    $result = mysql_query($sql, $conn) or die(mysql_error());
                    $numrows = mysql_num_rows($result);
                    ...
                    if ($numrows > 0) {
                        ...
                        while ($rs = mysql_fetch_array($result)) {
                            ...
                            echo "</td>";
                            echo "<td>$id_num</td>";
                            echo "<td>" . number_format($commission, 0, ".", ",") . ($commission_type == "P" ? "%" : "") . "</td>";
                            echo "<td>";

                            if ($canModify) {
                                echo "<a href='index.php?ss=$ss&mod=dealers.add&id=$id&cmd=EDIT'>Modify</a> | ";
                            }
                            ...
                            if ($canView) {
                                echo "<a href='index.php?ss=$ss&mod=dealers.view&dealer_id=$id'>View</a>";
                            }
