<?
    ...
    if (checkPermission($userInfo["id"], MODIFY)) {
        if ($cmd == "UPDATE") {
            if (!empty($id)) {
                $sql = "update users_tasks set status='COMPLETE', updated=NOW() where id='$id'";
                $result = mysql_query($sql, $conn) or die(mysql_error());

                setMessage("Task has been updated successfully.", true);
                system_log(MODIFY, "Task updated.", $userInfo["id"]);
                unset($cmd, $id);
            } else {
                setMessage("One or more required fields are empty.");
        ...
        <td>
            ...
            <? if (checkPermission($userInfo["id"], VIEW)) { ?>
                <table border="0" width="100%" cellpadding="5" cellspacing="0" align="center" id="reclist" style="margin-top: 10px;">
                    ...
                    <?
                        $sql = "select * from users_tasks where user_id='" . $userInfo["id"] . "' order by status asc, created asc";
                        $result = mysql_query($sql, $conn) or die(mysql_error());
                        ...
                        if ($numrows > 0) {
                            ...
                            while ($rs = mysql_fetch_array($result)) {
                                ...
                                if ($canModify) {
                                    if ($status == "DUE") {
                                        echo "<a style='float: left; margin: 5px;' href='index.php?ss=$ss&mod=$mod&id=$id&cmd=UPDATE'>Completed?</a>";
                                    } else {
